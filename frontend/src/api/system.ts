 ...
 * 获取 LLM 配置
 */
export function getLlmConfig(): Promise<ApiResponse<LlmConfig>> {
 ...
 * 更新 LLM 配置
 */
export function updateLlmConfig(data: Partial<LlmConfig>): Promise<ApiResponse<LlmConfig>> {
 ...
 * 测试 LLM 连接
 */
export function testLlmConnection(data: Partial<LlmConfig>): Promise<ApiResponse<{ success: boolean; message: string }>> {
 ...
 * 获取系统日志
 */
export function getLogs(params: LogQuery): Promise<ApiResponse<LogResult>> {
